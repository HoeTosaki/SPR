...
# 创建size大小的完全图，但调用from_adj做了很多无用功。
def clique(size):
...
def parse_adjacencylist(f):
    ...
    for l in f:
        if l and l[0] != "#":
            ...
            row = [introw[0]]
            row.extend(set(sorted(introw[1:])))  # 为何先拿出row0？
            adjlist.extend([row])
...
def _write_walks_to_disk(args):
  ...
  G = __current_graph
  if __current_graph is None:
      print('system warning for reload disk graph named <current_graph>')
      g,_ = dgl.load_graphs('../tmp/current_graph.tmp')
      g = g[0]
      G = Graph()
      for node in g.nodes().tolist():
          G[node] = g.successors(node).tolist()
      G.make_consistent()
...
#     emb_sz = 128
#     window_sz = 10
#     out_dir = '../outputs/dw-blogcatalog'
#     workers = 16
#     deepwalk = DeepWalkManagee(g=g,num_walks=num_walks,walk_lens=walk_lens,emb_sz=emb_sz,window_sz=window_sz,out_dir=out_dir,workers=workers)
#     deepwalk.train()